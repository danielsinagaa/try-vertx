package com.mandiri.try_vertx;

import com.mandiri.try_vertx.config.Runner;
...
public class MainVerticle extends AbstractVerticle {
  ...
  public void start(Promise<Void> startPromise) throws Exception {
    ...
    vertx.createHttpServer().requestHandler(router).listen(8078, http -> {
      if (http.succeeded()) {
        startPromise.complete();
        System.out.println("HTTP server started on port 8078");
      } else {
  ...
  private void setUpInitialData() {
    ...
    client = JDBCPool.pool(vertx, new JsonObject()
      .put("url", "jdbc:postgresql://localhost:5432/vertxtest")
